...
dec13 <- filter(rcv, 
                as.Date("2013-12-01") >= sdate & as.Date("2013-12-01") <= edate,
                !is.na(lat),
                !is.na(lon)) %>%
  st_as_sf(coords = c("lon", "lat"), crs = 4326) %>%
...
then <- ggplot() + theme_bw(base_size = 12) +
  ...
  geom_point(data = sf_as_df(dec13), aes(x = lon, y = lat),
             size = 1, pch = 21, color = "black", fill = "red") +
  labs(title = "(a) 1 December 2013") +
...
fig <- then + now
ggsave("Output/fig1_motus_changes_2013_2019.jpg", device = "jpeg",
